...
prep_x <- function(x, verbose = TRUE) {
  ...
  if (any(idx_integer)) {
    ...
    if (any(to_numeric)) {
      if (verbose) {
        warning(
          "Recoding integers with more than 5 unique values as numeric. ",
    ...
    if (any(to_factor)) {
      if (verbose) {
        warning(
          "Recoding integers with fewer than 6 unique values as ordered factors. ",
...
cforde <- function(params,
  ...
  update_fun <- function(step_) {
    ...
    cnt_conds <- condition_long_step[variable %in% cnt_cols, c("c_idx", "variable", "min", "max", "val")][, `:=`(
  ...
  cvg_new <- unique(
    rbind(
      cat_new[, .(f_idx, c_idx, variable, cvg_factor)],
...
prep_cond <- function(evidence, params, row_mode) {
  ...
  if (nrow(cond_lnot) > 0) {
    ...
    condition_long <- rbind(
      condition_long[!((variable %in% cat_cols) & str_detect(val, "^!")), ],
      cond_lnot
    )
  }
  ...
  condition_long[
    (variable %in% cnt_cols) & str_detect(val, "\\("),
