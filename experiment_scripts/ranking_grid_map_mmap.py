...
def create_colored_latex_table(
    ...
    # Header
    latex_str += "Dataset - Dimension & 20\% Q, 80\% E & 50\% Q, 50\% E & 20\% Q, 50\% E, 30\% V & 50\% Q, 30\% E, 20\% V \\\\n"
    latex_str += r"\midrule" + "\n"
    ...
    latex_str += r"\midrule" + "\n"
    latex_str += r"\quad Times Ranked Highest: & "
...
latex_table = create_colored_latex_table(
    ...
    rank1_counts,
    condition_order,
    caption='Average rankings of methods by MPE and MMAP estimates across ten trials. Winning entries in bold. Underlined results denote at least one timeout. In these cases, the target PAC guarantee is not attained, but weaker, Pareto-optimal certificates can be generated via the fixed-budget algorithm $\pi_{\mathrm B}$. Missing results indicate the method could not produce estimates due to technical limitations.',
